...
/**
 * Log in screen (Figma 144:64). Same shell as SignUp: red + pattern header, white sheet.
 * Icons: Flaticon Uicons Regular Rounded (`fi-rr-*`).
 */
export default function LogIn() {
  ...
  async function handleForgotPassword() {
    ...
    try {
      ...
      setInfo("If that email is registered, you will receive a reset link.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      ...
      `}</style>
      <div
        className="login-viewport relative isolate w-full overflow-hidden"
      ...
      >
        ...
        <div className="login-viewport relative z-10 flex w-full min-w-0 flex-col">
          {/* Taller red band vs SignUp (~246px in Figma frame) */}
          <div
            className="shrink-0"
            style={{ minHeight: "clamp(120px, 32vw, 246px)" }}
            aria-hidden
          />

          <main className="mt-35 flex w-full min-w-0 flex-1 flex-col rounded-tl-[49px] rounded-tr-[49px] bg-[#fafafa] px-[25px] pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-12">
            <h1
              className="mx-auto mb-10 max-w-[280px] text-center text-[40px] leading-[37px] tracking-[-1px] text-[#d51017]"
              style={{
            ...
            <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
              <div className="relative">
                <i
                  className="fi-rr-envelope login-field-icon pointer-events-none absolute left-[15px] top-1/2 -translate-y-1/2"
                  aria-hidden
                />
                <input
              ...
              <div className="relative">
                <i
                  className="fi-rr-lock login-field-icon pointer-events-none absolute left-[15px] top-1/2 -translate-y-1/2"
                  aria-hidden
                />
                <input
              ...
              <button
                ...
                className="mx-auto py-1 font-normal text-[#583921] underline-offset-2 transition-opacity enabled:hover:underline enabled:hover:opacity-90 disabled:opacity-50"
                style={{ fontFamily: "'Nunito', system-ui, sans-serif" }}
              >
                Forgot Password?
              </button>
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-2 flex h-[52px] w-full cursor-pointer items-center justify-center rounded-[30px] bg-[#ff7be2] text-lg font-medium text-[#fafafa] shadow-[0px_3px_4px_rgba(76,76,76,0.25)] transition-transform enabled:hover:scale-[1.02] enabled:active:scale-[0.98] disabled:opacity-60"
                style={{ fontFamily: "'Nunito', system-ui, sans-serif" }}
              >
            ...
            <p
              ...
              style={{ fontFamily: "'Nunito', system-ui, sans-serif" }}
            >
              <span className="text-[#8a8686]">Don&apos;t have an account?</span>
              <Link
                to="/signup"
